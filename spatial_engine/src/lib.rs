...
pub extern "C" fn compute_resolutions_batch(
    ...
    if lats.is_null() || lngs.is_null() || out_res2.is_null() || out_res3.is_null() || out_res4.is_null() || out_res6.is_null() || out_res7.is_null() {
        return;
    }
    ...
    unsafe {
        ...
        let res3_slice = slice::from_raw_parts_mut(out_res3, count);
        ...
        for i in 0..count {
            if let Ok(ll) = LatLng::new(lats_slice[i], lngs_slice[i]) {
                ...
                res3_slice[i] = ll.to_cell(h3o::Resolution::Three).into();
